...
sudo bash -c 'debconf-set-selections <<< \"mysql-community-server  mysql-community-server/root-pass password root\"'
sudo bash -c 'debconf-set-selections <<< \"mysql-community-server  mysql-community-server/re-root-pass password root\"'
sudo bash -c 'DEBIAN_FRONTEND=noninteractive dpkg -R --install mysql/'
...
sudo cp /tmp/shared/nagios.cfg /etc/apache2/conf-available/nagios.conf
